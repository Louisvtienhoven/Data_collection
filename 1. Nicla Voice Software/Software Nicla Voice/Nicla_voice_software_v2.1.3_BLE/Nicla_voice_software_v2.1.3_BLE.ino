 ...
 * Modified to accept measurement command via BLE.
 *
 ...
 *
 *   - WAITING_FOR_COMMAND: The device waits for a measurement command via Serial or BLE.
...
// BLE Service and Characteristic UUIDs – the characteristic UUID must match the Python script.
#define MEASUREMENT_SERVICE_UUID "12345678-1234-5678-1234-56789abcdef0"
#define COMMAND_CHAR_UUID        "abcdef01-1234-5678-1234-56789abcdef0"
BLEService measurementService(MEASUREMENT_SERVICE_UUID);
BLECharacteristic commandCharacteristic(COMMAND_CHAR_UUID, BLEWrite, 64);
...
// --------------------
// Process a command (from Serial or BLE)
// --------------------
...
// --------------------
// Setup: Initialize SPI Flash, LittleFS, Nicla, BMI270, BLE, etc.
// --------------------
void setup() {
  Serial.begin(115200);
  delay(3000);
  ...
  // ----- BLE Initialization (NEW) -----
  if (!BLE.begin()) {
    Serial.println("Starting BLE failed!");
    while (1);
  }
  // Set device name so that the Python script can find "NiclaVoice1" (or change as needed)
  BLE.setLocalName("NiclaVoice1");
  BLE.setAdvertisedService(measurementService);
  measurementService.addCharacteristic(commandCharacteristic);
...
// --------------------
void loop() {
  // In the WAITING_FOR_COMMAND state, check both Serial and BLE for commands.
  if (currentState == WAITING_FOR_COMMAND) {
    ...
    if (central) {
      if (commandCharacteristic.written()) {
        String command((const char*)commandCharacteristic.value());
        command.trim();
    ...
    // If this is the first pass in SAMPLING, initialize startTime now.
    if (startTime == 0) {
      startTime = millis();
    }
