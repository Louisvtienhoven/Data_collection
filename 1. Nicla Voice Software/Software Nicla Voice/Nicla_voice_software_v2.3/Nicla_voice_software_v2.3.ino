 ...
 *
 * The measurement command (format: frequency,realWorldTime,duration,remove_flag,extraDelayMin)
...
bool loadMeasurementCommand() {
  ...
  // Expected format: "frequency,realWorldTime,duration,remove_flag,extraDelayMin"
  ...
  String removeStr, extraDelayStr;
  if (fourthComma < 0) {
    // Only 4 parameters provided.
    removeStr = command.substring(thirdComma + 1);
    extraDelayMin = 0;
  } else {
    removeStr = command.substring(thirdComma + 1, fourthComma);
    extraDelayStr = command.substring(fourthComma + 1);
...
void setup() {
  ...
  if (loadMeasurementCommand()) {
    ...
    Serial.println("  frequency,realWorldTime,duration,remove_flag,extraDelayMin");
    Serial.println("Example: 65,1708531200,0.05,1,2");
    ...
    if (Serial.available() > 0) {
      ...
      if (command.length() > 0) {
        ...
        if (firstComma < 0 || secondComma < 0 || thirdComma < 0) {
          ...
          Serial.println("Command format: frequency,realWorldTime,duration,remove_flag,extraDelayMin");
        } else {
          String freqStr = command.substring(0, firstComma);
          String timeStr = command.substring(firstComma + 1, secondComma);
          String durStr = command.substring(secondComma + 1, thirdComma);
