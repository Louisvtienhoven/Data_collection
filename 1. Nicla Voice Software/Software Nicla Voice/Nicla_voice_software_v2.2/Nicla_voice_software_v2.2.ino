 ...
 *
 * The measurement command (format: frequency,realWorldTime,duration,waiting_time,remove_flag)
 * can be stored in flash.
 */
...
void writeMetadataToFile() {
  ...
  int n = snprintf(metadata, sizeof(metadata),
                   ...
                   sampleFrequencyHz, sampleDurationMin, waitTimeMin, readableTime.c_str());
...
bool loadMeasurementCommand() {
  ...
  // Expected format: "frequency,realWorldTime,duration,waiting_time,remove_flag"
  ...
  Serial.print("  Waiting time (min): "); Serial.println(waitTimeMin);
...
void setup() {
  ...
  if (loadMeasurementCommand()) {
    ...
    if (g_waitTimeMs > 0) {
      ...
      Serial.print(waitTimeMin);
      Serial.println(" minutes before sampling starts...");
      delay(g_waitTimeMs);
    }
    ...
    Serial.println("  frequency,realWorldTime,duration,waiting_time,remove_flag");
    Serial.println("Example: 65,1708531200,0.05,0.2,1");
    ...
    if (Serial.available() > 0) {
      ...
      if (command.length() > 0) {
        ...
        if (firstComma < 0 || secondComma < 0 || thirdComma < 0 || fourthComma < 0) {
          ...
          Serial.println("Command format: frequency,realWorldTime,duration,waiting_time,remove_flag");
        } else {
          String freqStr = command.substring(0, firstComma);
          String timeStr = command.substring(firstComma + 1, secondComma);
          String durStr = command.substring(secondComma + 1, thirdComma);
          ...
          Serial.print("Waiting ");
          Serial.print(waitTimeMin);
          Serial.println(" minutes for cable disconnection and power source change...");
          delay(g_waitTimeMs);
