// Include Arduino framework and libraries
#include <Arduino.h>
...
// BLE definitions
// ---------------------------------------------------
// Define a BLE Service and a Write Characteristic
BLEService commandService("12345678-1234-5678-1234-56789abcdef1");
BLECharacteristic commandCharacteristic("abcdef01-1234-5678-1234-56789abcdef0", BLEWrite, 64);
...
// ---------------------------------------------------
// Process a command string (from Serial or BLE)
// ---------------------------------------------------
...
// ---------------------------------------------------
// Setup: Initialize SPI Flash, LittleFS, BMI270, Serial, and BLE.
// ---------------------------------------------------
void setup() {
  Serial.begin(115200);
  delay(3000);
  ...
  // Check if measurement was already completed by looking for the flag file.
  {
    ...
    if (doneFile.open(&fs, "done.txt", O_RDONLY) == 0) {
      ...
      // Removed stray break; statement here.
    }
  }
...
// ---------------------------------------------------
// Loop: Process commands via Serial and BLE and perform sampling.
// ---------------------------------------------------
void loop() {
  // Check for BLE central connections and process any written commands
  BLEDevice central = BLE.central();
  if (central) {
    // While the central is connected:
    while (central.connected()) {
      // If a command is written via BLE, process it.
      if (commandCharacteristic.written()) {
        // Cast the value to const char* to avoid conversion error.
        String bleCommand = String((const char*)commandCharacteristic.value());
        bleCommand.trim();
        Serial.print("Received BLE command: ");
        Serial.println(bleCommand);
        // Process the command as long as we are waiting for a command.
        if (currentState == WAITING_FOR_COMMAND) {
          processCommand(bleCommand);
        }
      }
      // Also process Serial commands if available.
      if (Serial.available() > 0 && currentState == WAITING_FOR_COMMAND) {
        String serialCommand = Serial.readStringUntil('\n');
        serialCommand.trim();
        // Check for retrieval commands.
        if (serialCommand.equalsIgnoreCase("GETCSV")) {
          Serial.println("Sending CSV file contents:");
          readAndPrintFile(LOG_FILENAME);
          Serial.println("=== End of CSV ===");
        }
        else if (serialCommand.equalsIgnoreCase("CLRFLAG")) {
      ...
      // Once in the SAMPLING or FINISHED state, break out to handle them below.
      if (currentState != WAITING_FOR_COMMAND) break;
      
      BLE.poll();
    }
  }
  
  // If no BLE central is connected, also check Serial commands when waiting for command.
  if (currentState == WAITING_FOR_COMMAND && Serial.available() > 0) {
    String command = Serial.readStringUntil('\n');
    command.trim();
    // Process known retrieval commands if needed
